var backgroundTask = function () {
    var cacherAPI = function () {
        var cacheFolder = Windows.Storage.ApplicationData.current.localFolder;
        var get = function (params) {
            return checkCache(params).then(function (cachedUrl) {
                var url;
                if (!cachedUrl) {
                    url = 'ms-appx:///assets/noscreen.png';
                    return { "src": url, "params": params };
                }
                else {
                    url = cachedUrl.split('\\');
                    url = "ms-appdata:///local/" + url[(url.length - 1)];
                    return { "src": url, "params": params };
                }
            }, function (res) {
                var url = 'ms-appx:///assets/noscreen.png';
                return { "src": url, "params": params };
            });
        };
        var checkCache = function (params) {
            return cacheFolder.getItemAsync(params.imgid + '.jpg').then(function (file) {
                return file;
            }, function () {
                return getImages(params).then(function (res) {
                    return saveToCache(res.fanart.small, cacheFolder, params.imgid);
                });
            }).then(function (file) {
        ...
        /**
            ...
            */
            ...
            var upcomingData;
                        ...
                        if (xmlhttp.status === 200) {
                            ...
                            if (upcomingData.length > 0) {
                                ...
                                for (var b = 0; b < upcomingData.length; b++) {
                                    var params = { "tmdb": upcomingData[b].show.ids.tmdb, "imgid": "show-fanartsmall-" + upcomingData[b].show.ids.trakt, "imgtype": "showfanartsmall", "index":b };
                                    cacherAPI.get(params).then(function (result) {
                                        var a = result.params.index;
                                        var data = upcomingData;
                                        var newurl = result.src;
                                        ...
                                        if (tilesAdded < 4) {
                                            ...
                                            if (secondDate >= firstDate) {
                                                ...
                                                '<text />' +
                                                '<text hint-style="captionSubtle">Season ' + data[a].episode.season + ' Episode ' + data[a].episode.number + '</text>' +
                                                '<text hint-style="captionSubtle" hint-wrap="true">' + data[a].episode.title + '</text>' +
                                                '</binding>' +
                                                '</visual>' +
                                                '</tile>';
                ...
                var urltoget = "https://api.trakt.tv/calendars/my/shows/" + startdate + "/" + ((Windows.Storage.ApplicationData.current.roamingSettings.values['trk_myshows_days']) ? Windows.Storage.ApplicationData.current.roamingSettings.values['trk_myshows_days'] : 7) + "?extended=full,images&v=" + Date.now();
